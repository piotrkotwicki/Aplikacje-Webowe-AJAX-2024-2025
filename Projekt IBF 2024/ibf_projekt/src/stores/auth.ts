...
export const useAuthStore = defineStore({
  ...
  state: () => ({
    ...
    authority: localStorage.getItem('authority') ? JSON.parse(localStorage.getItem('authority')!) : null,
  ...
  actions: {
    async login(username: string, password: string) {
      ...
      if (response.ok) {
        ...
        const authorityResponse = await fetch('http://localhost:8080/authority/' +nazwa, {
          method: 'GET',
          headers: {
